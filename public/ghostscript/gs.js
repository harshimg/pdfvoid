...
if (ENVIRONMENT_IS_NODE) {
    ...
    // Updated to use Fetch API for reading text files
    Module["read"] = async function shell_read(url) {
    ...
    if (ENVIRONMENT_IS_WORKER) {
        // Updated to use Fetch API for reading binary files
        Module["readBinary"] = async function readBinary(url) {
            const response = await fetch(url);
            if (!response.ok) {
                throw new Error("Failed to fetch " + url + ". Status: " + response.status);
            }
    ...
    // Updated to use Fetch API for asynchronous reads
    Module["readAsync"] = function readAsync(url, onload, onerror) {
        fetch(url)
            .then((response) => {
                if (response.ok || (response.status === 0 && response.body)) {
...
// Load the WASM binary file using a promise-based approach
// Prefix of data URIs emitted by SINGLE_FILE and related options
...
function isDataURI(filename) {
    if (!filename || typeof filename !== 'string') {
        console.error("Invalid filename passed to isDataURI:", filename);
        return false;
    }
...
async function loadWasm() {
    return import("./gs.wasm?url")
        .then((wasmModule) => {
            if (!wasmModule || !wasmModule.default) {
                console.error("WASM module is invalid or not properly loaded:", wasmModule);
                throw new Error("WASM binary file not loaded properly");
            }

            wasmBinaryFile = wasmModule.default; // Access the default export from the module
            console.log("WASM binary file loaded:", wasmBinaryFile);
            return wasmBinaryFile;
        })
        .catch((error) => {
            console.error("Error loading the WASM file:", error);
        });
}

// Initialize the WASM module when needed
async function initializeWasmModule() {
    try {
        // Wait for the WASM file to load
        await loadWasm();

        if (!wasmBinaryFile) {
            throw new Error("Failed to load the WASM binary file");
        }

        // Additional code to use the wasmBinaryFile after it's loaded...
...
// Function to get the binary data
function getBinary() {
    try {
...
function createWasm(env) {
    ...
    return {}; // No exports yet, they will be filled in later
}
...
var FS = {
    ...
    createLazyFile: function (parent, name, url, canRead, canWrite) {
        ...
        LazyUint8Array.prototype.cacheLength = async function LazyUint8Array_cacheLength() {
            ...
            var datalength = Number(response.headers.get("Content-length"));
            var hasByteServing = response.headers.get("Accept-Ranges") === "bytes";
            var usesGzip = response.headers.get("Content-Encoding") === "gzip";

            let chunkSize = 1024 * 1024; // Chunk size in bytes

            if (!hasByteServing) chunkSize = datalength;

            // Function to get a range from the remote URL using Fetch API
            var doFetch = async function (from, to) {
                if (from > to)
                    throw new Error("Invalid range (" + from + ", " + to + ") or no bytes requested!");
                if (to > datalength - 1)
                    throw new Error("Only " + datalength + " bytes available! programmer error!");

                // Fetch the byte range using Fetch API
                ...
                if (!rangeResponse.ok) {
                    throw new Error("Couldn't load " + url + ". Status: " + rangeResponse.status);
                }
            ...
            lazyArray.setDataGetter(async function (chunkNum) {
                const start = chunkNum * chunkSize;
                let end = (chunkNum + 1) * chunkSize - 1; // including this byte
                end = Math.min(end, datalength - 1); // Make sure the end is within the file length

                if (typeof lazyArray.chunks[chunkNum] === "undefined") {
                    lazyArray.chunks[chunkNum] = await doFetch(start, end);
                }
            ...
            if (usesGzip || !datalength) {
                // If the server uses gzip or doesn't supply the length, download the whole file to get the length
                chunkSize = datalength = 1; // This will force getter(0)/doFetch to download the whole file
                datalength = (await lazyArray.getter(0)).length;
